...
float plane(vec3 p) {
    ...
    return p.y +.5 + sinbumps(p-vec3(time*10.,.0,.0), .25)*1 + sinbumps(p, 2.)*.2 + sinbumps(p, 10.)*.09;
}
...
float tooth3(in vec3 p) {
    ...
    p.x += sin(pp.y*2.)*.03;
    p.z += sin(pp.y*8.)*.01*mov;
    float b3 = capsule(p-vec3(x+.4,.9,z+.25), .1, .8);
    p.x += sin(pp.y*7.)*.05;
    p.z += sin(pp.y*6.)*.03;
    float b4 = capsule(p-vec3(x+.4,.9,z-.4), .09, .8);
    p = pp;

    p.x -= sin(p.y)*mov/2.-.25;
    p.z += cos(p.y*7.*p.x*mov1/2.)*.1;
    float s = sphere(p-vec3(x,2.4-mov/4.,z), .8);
    p = pp;
...
float scene(vec3 p) {
    return min(plane(p), min(tooth1(p), min (tooth2(p), tooth3(p))));
}
...
void main() {
    ...
    if (step > 0.) {
        ...
        if (plane(p) < eps) {
            float checker = mod(floor(p.x+time*10.)+floor(p.z)-.5, 2.0);
        ...
        if (tooth3(p) < eps) {
            ...
            m1.shininess = 3.;
            s1 = get_shading(m1, l1, p, n, ld, ed);
            color = m1.color * s1.diffuse * s1.shadow/.5;
            color += s1.specular;
            color *= s1.aoc;
            color += m1.color * s1.amb *.2;
        }
