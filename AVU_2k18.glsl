...
void main(void) {
    ...
    for( int lol=1; lol < 7; ++lol ) {
        for( int i=1; i < 5; ++i ) {
            c+=circ(lol*.2-.1,i*.2,m1*.02,cw);
        }
    }
	...
	if (mx82 == 1.) {
		...
		cx = mix(cx,vec3(mx84,mx85,mx86),mx81);		//this is not really doing colorize
	} else {
		cx = vec3(cx.r*mx84,cx.g*mx85,cx.b*mx86);
	}
