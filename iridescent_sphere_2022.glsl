...
float iridescent_sphere3(vec3 p, float bumps) {
    return length(p-vec3(1.2,1.,-1.2)) + bumps -.7;
}

float iridescent_sphere4(vec3 p, float bumps) {
    return length(p-vec3(-1.2,1.,-1.)) + bumps -.7;
}
...
float scene(vec3 p) {
    ...
    return min(min(min(min(iridescent_sphere4(p, bumps), 
        iridescent_sphere3(p, bumps)), iridescent_sphere2(p, bumps)), iridescent_sphere1(p, bumps)), plane(p));
}
...
void main() {
    vec3 ro = vec3(cos(time/4.)*3., 2.5, sin(time/4.)*3.);
    ...
    for (int i = 0; i < MAXSTEPS; ++i) {
        ...
        if (d < eps) {
            ...
            if (iridescent_sphere1(p, bumps) < eps) {
                vec3 perturb = sin(p * 300.);
                m1.color = spectrum( dot(perturb * .05 + n, ro) * 2.);
                s1 = get_shading(m1, l1, p, n, ld, ed);
                color = 0.1 - m1.color;
                color += clamp(.5 * pow(dot(normalize(reflect(-ld, n)), ed), 7.), 0., 1.);
            ...
            if (iridescent_sphere3(p, bumps) < eps) {
                vec3 perturb = sin(p * 50.);
                m1.color = spectrum( dot(perturb * .05 + n, ro) * 2.);
                s1 = get_shading(m1, l1, p, n, ld, ed);
                color = 0.1 - m1.color;
                color += clamp(.9 * pow(dot(normalize(reflect(-ld, n)), ed), 7.), 0., 1.);
                color = pow( color, vec3(1.0/10.2) );
            }
            if (iridescent_sphere4(p, bumps) < eps) {
                vec3 perturb = sin(p * 10.);
                m1.color = spectrum( dot(perturb * .05 + n, ro) * 2.);
                s1 = get_shading(m1, l1, p, n, ld, ed);
                color = 0.5 - m1.color;
                color += clamp(2. * pow(dot(normalize(reflect(-ld, n)), ed), 7.), 0., 1.);
            }
            if (plane(p) < eps) {
