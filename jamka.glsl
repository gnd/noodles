...
void main(void) {
	...
	float distort = mx16 + m1*mx21; 		// height of sinx bumps controlled by 21
	...
	if (outside == 0.) {
		c += lights(camPos, sp, surfNormal)*mx12*m0;
	}
	...
	// ZRPT'S
	// this wont work coz of m51 that has to be off, fuck !! solve this !!!
	if (mx42 == 1.) {
	...
	if (mx62 == 1.) {
		...
		if (mx63 == 0.) {
			for (int i=0; i<10; i++) {
				cx = feedb_sqr(float(i)/10.+0.05, yp, xs, ys, bs, 0., 1., c);
				c = mix(c,cx,mx61);
			}
		}
		if (mx63 == 1.) {
			...
			for (int i=1; i<=int(ys); i++) {
       				cx = feedb_crc(spacing*i-spacing/2., yp, xs, bs, c);
				c = mix(c,cx,mx61);
			} 
		}
		//c = mix(c,cx,mx61);
