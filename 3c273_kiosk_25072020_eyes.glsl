...
float scene(vec3 p) {
    ...
    for (int i = 0; i < 3; i++ ) {
        dst = min(dst, eye(p, c -vec3(0.,mx11*i, mx21*10.+rand.y), 1.));
    }
...
for (int i = 0; i < 3; i++ ) {
        if (sclera(p, vec3(0.,.9,.0)-vec3(0.,mx11*i,mx21*10.+rand.y), 1.) < eps) {
            mat1.color = vec3(1.)*2.5;
            mat1.reflection_ratio = 1.01;
            mat1.shininess = 50.;
        ...
        if (iris(p, vec3(0.,.9,.0)-vec3(0.,mx11*i,mx21*10.+rand.y), 1.) < eps) {
            ...
            mat1.color = vec3(10.1, 0.2, 2.9);
            mat1.color *= eyp((oo+vec2(0.,-.35))*100.)*5.;
            mat1.color *= eyp((oo+vec2(0.,-.35))*10.)*15.;
            mat1.reflection_ratio = 10.01;
            mat1.shininess = 350.;
            s1 = get_shading(mat1, l1, p, n, ld, ed);
            color = mat1.color * s1.diffuse * s1.shadow/2.;
            color += s1.specular;
        ...
        if (pupil(p, vec3(0.,.9,.0)-vec3(0.,mx11*i,mx21*10.+rand.y), 1.) < eps) {
            color = vec3(0.);
        }
    }
    }
    ...
    if (mx72 > 0.) {
        color *= feedb_sqr(mx76, mx75, mx66, mx65, mx74, color)*mx71*1.05;
    }
