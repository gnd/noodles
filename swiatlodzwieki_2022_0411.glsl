...
float refractor(vec3 p, float bumps) {
    // set size (using MAD notation)
    //vec3 q = p * -mx21 + p;
    vec3 q = p; 

    // rotate along the Y axis
    vec2 xz = R(q.xz, time*mx46+m4);
    p = vec3(xz.x, q.y, xz.y);

    // thickness from -.1 ==> 1
    float thick = mx24 * 8.1 - .1;
    float d = cbox(q/mx21*0.2, vec3(1.,thick,1.))*mx21*5. + bumps;
    float e = cbox(q/mx21*0.2, vec3(thick,1.,1.))*mx21*5. + bumps;
    float f = cbox(q/mx21*0.2, vec3(1.,1.,thick))*mx21*5. + bumps;
