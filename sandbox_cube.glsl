...
// all DF's below are an practical experimentantion with the box SDF from here
// https://www.alanzucconi.com/2016/07/01/signed-distance-functions/#part3


float hsp(vec3 p, vec3 cube) {
    return p.x - cube.x;
}

float hsm(vec3 p, vec3 cube) {
    return -p.x + cube.x;
}

float dhs(vec3 p) {
    return max(hsp(p,vec3(.8,0.,0.)), hsm(p,vec3(-.8,0.,0.)));
}

float hsmix(vec3 p, vec3 cube) {
    // we assume this is around origin
    float mid = cube.x / 2.;
    return max(p.x-mid, -mid-p.x);
}

float hsmixfull(vec3 p, vec3 cube) {
    // we assume this is around origin
    vec3 mid = cube * .5;
    vec3 res = vec3(max(p.x-mid.x, -mid.x-p.x), max(p.y-mid.y, -mid.y-p.y), max(p.z-mid.z, -mid.z-p.z));
    return maxcomp(res);
}

float cubeabs(vec3 p, vec3 cube) {
    // we assume this is around origin
    vec3 mid = cube * .5;
    return maxcomp(abs(p)-mid);
}


float scene(vec3 p) {
    // 1. return min(plane(p), max(box(p, vec3(100.,1.,1.),0.), dhs(p)));
    // 2. return min(plane(p), hsmix(p, vec3(0.1,0.01,0.01)));
    // 3 . return min(plane(p), hsmixfull(p, vec3(1.)));
    return min(plane(p), cubeabs(p, vec3(1.)));
}
