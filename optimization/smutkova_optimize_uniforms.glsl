...
void main() {
    ...
    */
    ...
    vec3 rd = normalize(fwd * mx41 + right * (uv.x*aspect) + up*uv.y);
    // enviroment
    //vec3 lpos = vec3(sin(time/4.)*4.,2.5,cos(time/4.)*3.);
    vec3 lpos = vec3(stime*4.,2.5,ctime*3.);
    color = vec3(.0);
    ...
    for (int i = 0; i < MAXSTEPS; i+=2) {
        ...
        if (d < eps) {
            ...
            color = vec3(clamp(dot(n,ld), 0.,.4)
                    ...
                    + n.y*.4+.01);
            // add step glow
            color.r += float(i)/float(MAXSTEPS)*.9;
            color /= texxcol*texxcol*texxcol;
            break;
        }
        step += d;
    }
    ...
    PixelColor = color;
    PixelColor = mix(color, vec3(0.), fogFactorExp2(step, .14));
