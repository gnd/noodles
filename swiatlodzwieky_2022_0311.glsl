...
float low_plane(vec3 p) {
    // bend
    float k = 0.05; // or some other amount
    float c = cos(k*p.x);
    float s = sin(k*p.x);
    mat2 m = mat2(c,-s,s,c);
    p = vec3(m*p.xy,p.z);
    return p.y + 30.+ noise2f(p.xz/10.)*10. + noise2f(p.xz) + noise2f(p.xz*5.) * 0.1;
}

float sky_plane(vec3 p) {
    // bend
    float k = -.05; // or some other amount
    float c = cos(k*p.x);
    float s = sin(k*p.x);
    mat2 m = mat2(c,-s,s,c);
    p = vec3(m*p.xy,p.z);
    return -p.y + 30. - noise2f(p.xz/10.)*10. + noise2f(p.xz) + noise2f(p.xz*5.);
}
...
float scene(vec3 p) {
    ...
    return min(min(c, min(hollow_sphere(p, bumps), d)), pla);
}
...
void main() {
    ...
    // TODO - this doesnt do what its supposed to do 
    //color = mix(color, 0.8 * sky, .6 - exp2(-0.010 * step * step));
